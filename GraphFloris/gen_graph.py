...
def update_edges(g, wind_direction, influence_angle_th, influence_dist_th):
    ...
    def compute_is_influential(edges):
        # 1. translate source and dst nodes so that source node places on the origin
        # 2. rotate the dst nodes counter clockwisely by 'wd' degree
        ...
        R = [[np.cos(wind_direction), -np.sin(wind_direction)],
             [np.sin(wind_direction), np.cos(wind_direction)]]  # ccw 'wd' matrix
        R = torch.tensor(np.stack(R)).float().float()  # [1, 2, 2]
        ...
        a = round(np.tan(np.radians(90 - influence_angle_th)), 10)
